...
mr_trash_whell, 105 observation for professor_trash_wheel,and 38
observation for Gwynnda_trash_wheel.

For `all_data`, there are 727 observation and key variables for these
four datasets are `weight` and `home_powered`.

The total weight of trash collected by `professor_trash_wheel` here is
214.21 and the total number of cigarette butts collected by Gwynnda in
