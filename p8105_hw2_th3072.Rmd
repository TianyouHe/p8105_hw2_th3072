...
584 observation for mr_trash_whell, 105 observation for professor_trash_wheel,and 38 observation for Gwynnda_trash_wheel.

For `all_data`, there are 727 observation and key variables for these four datasets are `weight` and `home_powered`. 
...
baseline <- baseline|>
  subset(age_at_study < age_onset_MCI_and_missing| age_onset_MCI_and_missing == ".")
```
baseline <- baseline|>
  subset(age_at_study < age_onset_MCI_and_missing| age_onset_MCI_and_missing = ".")
...
women_apoe4 <- nrow(women_carrier)/nrow(women)
```
